...
class Operator(object):
    ...
    def __init__(self,Lz,Nz,zType = 'Periodic',offset = None,BC1=None,BC2=None):
        '''
        ...
        if (zType == 'Periodic') :
            ...
            self.z = np.linspace(0,Lz,Nz,endpoint=True)
            dz          = Lz/(Nz-1)
            self.k      = self.get_wavenumbers(Nz,dz,zType)
            self.K      = np.abs(self.k)	
            self.K_filt = np.abs(self.k/np.amax(self.k))	
            ...
            self.Dz,self.z = CHEB.cheb(Nz-1,0,Lz)
            self.k = self.Dz
            self.BC1 = BC1
            self.BC2 = BC2
    ...
    def get_grad1D(cls,u,k,dim = 'z',zType='Periodic',order =1):
        ...
        if dim == 'x':
            ...
            if zType == 'Periodic':
                ...
                Fu_x = (np.sqrt(-1 +0j))**order*np.multiply(Fu,k**order)
                # Fu_x = (np.sqrt(-1 +0j))**order*Fu*(k**order)
