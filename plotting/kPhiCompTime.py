...
fig,axs = pl.subplots(1,3,figsize=(12,4),constrained_layout=True)
ax0 = axs[0]
ax1 = axs[1]
ax2 = axs[2]
...
simList = os.listdir(os.path.join('Analyses','kPhiSaddles'))
...
for i,simDataName in enumerate(simList[:]):
    ...
    simDataName = os.path.join('Analyses','kPhiSaddles',simDataName)
    ...
    if '0.01' in simDataName:
        colour = 'dodgerblue'
    elif '0.1' in simDataName:
        colour='seagreen'
    elif '1.0' in simDataName:
        colour = 'magenta'
    else:
        colour = 'k'
    ...
    if  '' in simDataName:
        if 'Diffusive' in simDataName:
            ax0.semilogy(eval_list,err_list,linestyle=ls,label=plotLabel[:20],color = colour)
        elif 'Per._Flux' in simDataName:
            ax1.semilogy(eval_list,err_list,linestyle=ls,label=plotLabel,color = colour)
        else:
            ax2.semilogy(eval_list,err_list,linestyle=ls,label=plotLabel[:20],color = colour)
...
## Labels
ax0.set_xlabel('Evaluations',fontsize=18)
ax0.set_ylabel(r'$J \ \mathrm{(^\circ C^2)}$',fontsize=18)
ax1.set_xlabel('Evaluations',fontsize=18)
ax2.set_xlabel('Evaluations',fontsize=18)
# ax1.set_ylabel(r'Error $J = \int\int(T-T_m)^2 dzdt$',fontsize=18)
...
ax0.legend(by_label.values(), by_label.keys(),fontsize=16,loc='upper right')#, bbox_to_anchor=(1, 1))
...
ax0.set_title('Case A: Diffusive',fontsize=20)
ax1.set_title('Case B: Periodic Flux',fontsize=20)
ax2.set_title('Case C: Random Flux',fontsize=20)

ax0.text(-5,2e-6,'a)',fontsize=16)
ax1.text(10,2e-6,'b)',fontsize=16)
ax2.text(10,2e-6,'c)',fontsize=16)
